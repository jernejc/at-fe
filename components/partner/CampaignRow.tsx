...
export function CampaignRow({ campaign, productName, companies, pipelineValue }: CampaignRowProps) {
    const router = useRouter();

    return (
        <div
            onClick={() => router.push(`/partner/${campaign.slug}`)}
            className="grid grid-cols-12 items-center gap-4 px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer">
            {/* Campaign Name */}
